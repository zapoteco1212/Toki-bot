...
// 🔥 Corregí el nombre a startModBot
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, isNewLogin, qr }) => {
    ...
    if (connection === 'close') {
      ...
      if ([401, 403].includes(reason)) {
        if (intentos < 5) {
          console.log(chalk.gray(`[ ✿  ]  MOD-BOT ${botId} Conexión cerrada (código ${reason}) intento ${intentos}/5 → Reintentando...`))
          setTimeout(() => {
            startModBot(m, client, caption, isCode, phone, chatId, {}, isCommand)
          }, 3000)
        } else {
          console.log(chalk.gray(`[ ✿  ]  MOD-BOT ${botId} Falló tras 5 intentos. Eliminando sesión.`))
          try {
